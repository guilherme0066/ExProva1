...
public class TermostatoController {
    ...
    public List<Termostato> buscarTodos() {
        return service.buscarTodosTermostatos();
    }

    @PostMapping
    public Termostato novoTermostato(@RequestBody Termostato termostato) {
        return service.novoTermostato(termostato);
    }
    
    @GetMapping(value = "/{id}")
    public Termostato buscarPorId(@PathVariable("id") Long id) {
